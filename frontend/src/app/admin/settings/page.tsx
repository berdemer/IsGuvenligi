...
export default function SettingsPage() {
  ...
  const handleAutoSave = useCallback(async () => {
    try {
      ...
      toast({
        ...
        description: t('toast.autoSavedDescription'),
      })
    } catch (error) {
      setState(prev => ({ ...prev, loading: false }))
      toast({
        title: t('toast.autoSaveFailed'),
        description: t('toast.autoSaveFailedDescription'),
        variant: "destructive"
      })
    }
  }, [toast])
  ...
  const handleReset = () => {
    ...
    toast({
      title: t('toast.settingsReset'),
      description: t('toast.settingsResetDescription'),
    })
  }
  ...
  const handleExportSettings = async () => {
    try {
      ...
      setState(prev => ({ ...prev, loading: false }))
      toast({
        title: t('toast.settingsExported'),
        description: t('toast.settingsExportedDescription'),
      })
    } catch (error) {
      setState(prev => ({ ...prev, loading: false }))
      toast({
        ...
        <div className="flex items-center gap-2">
          ...
          {state.lastSaved && (
            <span className="text-xs text-muted-foreground">
              {t('lastSaved')}: {new Date(state.lastSaved).toLocaleTimeString()}
            </span>
          )}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t('stats.systemStatus')}</p>
                <div className="flex items-center gap-2">
