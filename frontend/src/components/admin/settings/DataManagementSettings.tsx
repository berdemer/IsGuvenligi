...
export default function DataManagementSettings({ onSettingsChange }: DataManagementSettingsProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast({
        title: t('messages.exportCompleted'),
        description: t('messages.exportCompletedDesc'),
      })
    } catch (error) {
      toast({
        title: t('messages.exportFailed'),
        description: t('messages.exportFailedDesc'),
        variant: "destructive"
      })
    } finally {
      setLoading(false)
  ...
  const handleClearCache = async () => {
    ...
    try {
      ...
      toast({
        title: t('messages.clearFailed'),
        description: t('messages.clearFailedDesc'),
        variant: "destructive"
      })
    } finally {
      setLoading(false)
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>{t('backup.enableAutomated')}</Label>
              <p className="text-sm text-muted-foreground">
                {t('backup.enableAutomatedDesc')}
              </p>
            </div>
            <Switch
...
{t('export.button')}
          ...
          <div className="flex items-center justify-between">
            <div>
              <h4 className="font-medium">{t('import.title')}</h4>
              <p className="text-sm text-muted-foreground">
...
{t('cache.description')}
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>{t('cache.enableRedis')}</Label>
              <p className="text-sm text-muted-foreground">
                {t('cache.enableRedisDesc')}
              </p>
            </div>
            <Switch
...
{t('cleanup.title')}
        ...
        <CardContent className="space-y-4">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <Label>{t('cleanup.enableAutomatic')}</Label>
                <p className="text-sm text-muted-foreground">
                  {t('cleanup.enableAutomaticDesc')}
                </p>
              </div>
              <Switch
            ...
            {config.cleanup.enabled && (
              <div className="space-y-4 border-l-2 border-yellow-200 pl-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <Label>{t('cleanup.cleanTempFiles')}</Label>
                    <Switch
