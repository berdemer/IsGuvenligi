...
export function PolicyEditor({ 
  ...
  const validation = useMemo(() => {
    ...
    if (formData.resources.length === 0) errors.push("At least one resource is required")
    if (formData.priority < 1 || formData.priority > 100) errors.push("Priority must be between 1 and 100")
  ...
  const handleSimulation = async () => {
    if (!simulationUser.trim() || !simulationResource.trim()) {
      toast.error("Please enter both user and resource for simulation")
      return
    }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Define access rules, conditions, and rollout settings for your policy
          </DialogDescription>
        </DialogHeader>
