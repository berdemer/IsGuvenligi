...
export function SessionsList({ 
  ...
  const handleSessionAction = async (sessionId: string, action: SessionAction, options?: any) => {
    ...
    if (!permissions.canQuarantineIp && action === 'quarantine_ip') {
      toast.error("You don't have permission to quarantine IPs")
      return
    }
  ...
  const getActionSuccessMessage = (action: SessionAction): string => {
    switch (action) {
      case 'revoke': return 'Session revoked successfully'
      case 'revoke_block_ip': return 'Session revoked and IP blocked'
      case 'require_mfa': return 'MFA requirement added'
      case 'quarantine_ip': return 'IP quarantined successfully'
      case 'flag_compromised': return 'Session flagged as compromised'
      case 'require_reauth': return 'Reauthentication required'
  ...
  const getRiskBadge = (level: string, score: number) => {
    ...
    return (
      <Tooltip>
        ...
        <TooltipContent>
          <p>Risk Score: {score}/100 ({level})</p>
        </TooltipContent>
      </Tooltip>
    )
  }
        ...
        <CardContent className="p-6 space-y-4">
          <div className="flex items-center space-x-4">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search by email, username, IP, or device fingerprint..."
                value={searchTerm}
          ...
          {showFilters && (
            <div className="grid gap-4 md:grid-cols-4 p-4 bg-muted/50 rounded-lg">
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder="All MFA Status" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All MFA Status</SelectItem>
                  <SelectItem value="passed">MFA Passed</SelectItem>
                  <SelectItem value="required">MFA Required</SelectItem>
                  <SelectItem value="failed">MFA Failed</SelectItem>
                  <SelectItem value="bypassed">MFA Bypassed</SelectItem>
                </SelectContent>
              </Select>
            </div>
          )}
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2 text-xs text-muted-foreground">
                            ...
                            {session.location.isVpn && <Badge variant="outline" className="text-xs">VPN</Badge>}
                          </div>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      ...
                      {session.anomalies.length > 0 && (
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p>{session.anomalies.length} anomal{session.anomalies.length > 1 ? 'ies' : 'y'} detected</p>
                          </TooltipContent>
                        </Tooltip>
                      )}
                    </TableCell>
