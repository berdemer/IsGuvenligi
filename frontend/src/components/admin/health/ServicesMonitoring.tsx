...
const formatLastCheck = (timestamp: string, t: any): string => {
  ...
  if (diffMins < 1) return t('services.timeFormat.justNow')
  if (diffMins < 60) return t('services.timeFormat.minutesAgo', { minutes: diffMins })
  const diffHours = Math.floor(diffMins / 60)
  if (diffHours < 24) return t('services.timeFormat.hoursAgo', { hours: diffHours })
...
const ServiceDetail: React.FC<ServiceDetailProps> = ({ service, onClose }) => {
  ...
  const handleServiceAction = async (action: string) => {
    ...
    try {
      ...
      switch (action) {
        case 'restart':
          toast.success(t('services.toast.restarted', { serviceName: service.name }))
          break
        case 'clear_cache':
          toast.success(t('services.toast.cacheCleared', { serviceName: service.name }))
          break
        case 'health_check':
          toast.success(t('services.toast.healthCheckCompleted', { serviceName: service.name }))
          break
      }
    } catch (error) {
      toast.error(t('services.toast.actionFailed', { action, serviceName: service.name }))
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <span>{t('services.detail.type')}: {service.type}</span>
            {service.version && (
              <>
                <span>•</span>
                <span>{t('services.detail.version')}: {service.version}</span>
              </>
            )}
          </div>
        </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground">{t('services.detail.metrics.uptime7d')}</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground">{t('services.detail.metrics.avgLatency')}</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground">{t('services.detail.metrics.errorRate')}</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground">{t('services.detail.metrics.healthScore')}</div>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t('services.detail.metrics.currentMetrics')}</CardTitle>
        </CardHeader>
        <CardContent>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {metric.threshold && (
                      <Badge 
                        ...
                        className="text-xs"
                      >
                        {metric.value >= metric.threshold.critical ? t('services.threshold.critical') :
                         metric.value >= metric.threshold.warning ? t('services.threshold.warning') : t('services.threshold.good')}
                      </Badge>
                    )}
                  </div>
                </div>
      ...
      {dependencies.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t('services.detail.dependencies.title')}</CardTitle>
            <CardDescription>
              {t('services.detail.dependencies.description')}
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {recentIncidents.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t('services.detail.incidents.title')}</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {recentIncidents.map((incident) => (
                <div key={incident.id} className="border rounded p-3">
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    <span>{t('services.detail.incidents.duration', { duration: incident.duration })}</span>
                    <span>{t('services.detail.incidents.impact', { impact: incident.impact.userImpact })}</span>
                    <span>{t('services.detail.incidents.status', { status: incident.status })}</span>
                  </div>
                </div>
              ))}
            </div>
...
export default function ServicesMonitoring({
  ...
  const handleServiceAction = async (serviceId: string, action: string) => {
    try {
      toast.loading(t('services.toast.performing', { action }), { id: `${serviceId}-${action}` })
      
      // Simulate API call
      await new Promise(resolve => setTimeout(resolve, 2000))
      
      toast.success(t('services.toast.success', { action }), { 
        id: `${serviceId}-${action}` 
      })
    } catch (error) {
      toast.error(t('services.toast.error', { action }), { 
        id: `${serviceId}-${action}` 
      })
    }
  }
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="unhealthy-only" className="text-sm">
              {t('services.showUnhealthyOnly')}
            </label>
          </div>
          
          <div className="text-sm text-muted-foreground">
            {t('services.servicesCount', { filtered: filteredServices.length, total: services.length })}
          </div>
        </div>
      </div>
        ...
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{t('services.table.lastCheck')}</TableHead>
                <TableHead>{t('services.table.latency')}</TableHead>
                <TableHead>{t('services.table.errorRate')}</TableHead>
                <TableHead>{t('services.table.uptime')}</TableHead>
                <TableHead>{t('services.table.actions')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                    ...
                    <div className="flex items-center gap-2">
                      <Sheet>
                        ...
                        <SheetContent className="w-[600px] sm:max-w-[600px] overflow-y-auto">
                          <SheetHeader>
                            ...
                            <SheetDescription>
                              {selectedService && t('services.detail.description', { serviceName: selectedService.name })}
                            </SheetDescription>
                          </SheetHeader>
                          {selectedService && (
                            <div className="mt-6">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          >
                            <Activity className="h-4 w-4 mr-2" />
                            {t('services.actions.healthCheck')}
                          </DropdownMenuItem>
                          <DropdownMenuItem
                            onClick={() => handleServiceAction(service.id, 'clear_cache')}
                          >
                            <Trash2 className="h-4 w-4 mr-2" />
                            {t('services.actions.clearCache')}
                          </DropdownMenuItem>
                          <DropdownMenuItem
          ...
          {filteredServices.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-lg font-medium">{t('services.empty.title')}</p>
              <p className="text-sm">
                {searchTerm ? 
                  t('services.empty.searchMessage') : 
                  t('services.empty.filterMessage')
                }
              </p>
            </div>
          )}
