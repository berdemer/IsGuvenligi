          ...
          {usage !== undefined && (
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span>{t('infrastructure.usage')}</span>
...
const ServiceResourceUsage: React.FC<ServiceResourceUsageProps> = ({ services }) => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">{t('infrastructure.serviceResourceUsage')}</CardTitle>
        <CardDescription>
          {t('infrastructure.resourceConsumption')}
        </CardDescription>
      </CardHeader>
      <CardContent>
...
export default function InfrastructureMonitoring({ 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <ResourceCard
          title={t('infrastructure.cpuUsage')}
          ...
          details={[
            { label: t('infrastructure.cores'), value: metrics.system.cpu.cores.toString() },
            { label: t('infrastructure.loadAvg'), value: metrics.system.cpu.loadAverage.map(l => l.toFixed(1)).join(', ') }
          ]}
          trend={2.3}
        />
        
        <ResourceCard
          title={t('infrastructure.memory')}
          ...
          details={[
            { 
              label: t('infrastructure.swapUsed'), 
        ...
        <ResourceCard
          title={t('infrastructure.diskSpace')}
          ...
          details={[
            { 
              label: t('infrastructure.readIOPS'), 
              value: formatNumber(metrics.system.disk.iops?.read || 0) 
            },
            { 
              label: t('infrastructure.writeIOPS'), 
        ...
        <ResourceCard
          title={t('infrastructure.networkIO')}
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold">{t('infrastructure.historicalMetrics')}</h3>
          <Select value={selectedMetric} onValueChange={(value) => setSelectedMetric(value as any)}>
            <SelectTrigger className="w-40">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="cpu">{t('infrastructure.cpuUsage')}</SelectItem>
              <SelectItem value="memory">{t('infrastructure.memory')} {t('infrastructure.usage')}</SelectItem>
              <SelectItem value="disk">{t('infrastructure.diskSpace')} {t('infrastructure.usage')}</SelectItem>
              <SelectItem value="network">{t('infrastructure.networkIO')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          {selectedMetric === 'memory' && (
            <>
              <MetricChart
                title={`${t('infrastructure.memory')} ${t('infrastructure.usage')}`}
                data={memoryData}
                unit="%"
                color="#10b981"
              />
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t('infrastructure.memoryBreakdown')}</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span>{t('infrastructure.physicalMemory')}</span>
                    ...
                    <div className="flex justify-between items-center">
                      <span>{t('infrastructure.swapMemory')}</span>
          ...
          {selectedMetric === 'disk' && (
            <>
              <MetricChart
                title={`${t('infrastructure.diskSpace')} ${t('infrastructure.usage')}`}
                data={diskData}
                unit="%"
                color="#f59e0b"
              />
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t('infrastructure.diskIOPerformance')}</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center">
                      ...
                      <div className="text-sm text-muted-foreground">{t('infrastructure.readIOPS')}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">
                        {formatNumber(metrics.system.disk.iops?.write || 0)}
                      </div>
                      <div className="text-sm text-muted-foreground">{t('infrastructure.writeIOPS')}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          {selectedMetric === 'network' && (
            <>
              <MetricChart
                title={t('infrastructure.inbound')}
                data={networkInData}
                unit=" MB/s"
                color="#ef4444"
              />
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t('infrastructure.networkStatistics')}</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{t('infrastructure.inbound')}</span>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{t('infrastructure.errors')}</span>
                      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t('infrastructure.infrastructureAlerts')}</CardTitle>
          <CardDescription>
            {t('infrastructure.activeAlerts')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {metrics.system.memory.usage > 75 && (
              <div className="flex items-center gap-3 p-3 border rounded-lg bg-yellow-50">
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-sm">{t('infrastructure.highMemoryUsage')}</h4>
                  <p className="text-xs text-muted-foreground">
                    {t('infrastructure.memoryApproaching', { usage: metrics.system.memory.usage.toFixed(1) })}
                  </p>
                </div>
                <Badge variant="secondary">{t('status.warning')}</Badge>
              </div>
            )}
            ...
            {metrics.system.cpu.usage > 80 && (
              <div className="flex items-center gap-3 p-3 border rounded-lg bg-red-50">
                <AlertTriangle className="h-4 w-4 text-red-500" />
                <div className="flex-1">
                  <h4 className="font-medium text-sm">{t('infrastructure.criticalCPUUsage')}</h4>
                  <p className="text-xs text-muted-foreground">
                    {t('infrastructure.cpuImmediateAttention', { usage: metrics.system.cpu.usage.toFixed(1) })}
                  </p>
                </div>
            ...
            {metrics.system.disk.usage > 85 && (
              <div className="flex items-center gap-3 p-3 border rounded-lg bg-orange-50">
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-sm">{t('infrastructure.diskSpaceLow')}</h4>
                  <p className="text-xs text-muted-foreground">
                    {t('infrastructure.diskConsiderCleanup', { usage: metrics.system.disk.usage.toFixed(1) })}
                  </p>
                </div>
                <Badge variant="secondary">{t('status.warning')}</Badge>
              </div>
            ...
            {metrics.system.memory.usage <= 75 && 
             ...
             metrics.system.disk.usage <= 85 && (
              <div className="text-center py-6 text-muted-foreground">
                ...
                <p className="text-sm">{t('infrastructure.allMetricsNormal')}</p>
              </div>
            )}
          </div>
        </CardContent>
