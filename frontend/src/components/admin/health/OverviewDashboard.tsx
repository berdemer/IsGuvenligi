...
const SystemHealthGauge: React.FC<{ score: number; loading: boolean }> = ({ score, loading }) => {
  ...
  const getScoreStatus = (score: number) => {
    if (score >= 95) return t('healthScore.status.excellent')
    if (score >= 85) return t('healthScore.status.good')
    if (score >= 70) return t('healthScore.status.warning')
    return t('healthScore.status.critical')
  }
    ...
    <div className="flex flex-col items-center justify-center h-48">
      <div className="relative w-32 h-32">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-xs text-muted-foreground">{t('healthScore.outOf100')}</span>
        </div>
      </div>
      <div className="mt-4 text-center">
        ...
        <div className="text-xs text-muted-foreground">
          {useTranslations('health.overview.healthScore')('title')}
        </div>
      </div>
    </div>
  )
...
const ServiceUptimeCard: React.FC<{
  ...
  const t = useTranslations('health.overview.services')
  if (loading) {
    return (
      <Card>
        <CardHeader>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">{t('uptimeTitle')}</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          {services && services.length > 0 ? services.slice(0, 6).map((service) => (
            ...
            <div className="text-center py-6 text-muted-foreground">
              <p className="text-sm">{t('noServicesAvailable')}</p>
            </div>
          )}
        </div>
      </CardContent>
...
const TopAlertsCard: React.FC<{
  ...
  const t = useTranslations('health.overview.alerts')
  if (loading) {
    return (
      <Card>
        <CardHeader>
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline">
            {alerts.filter(a => !a.resolved).length} {t('active')}
          </Badge>
        </div>
      </CardHeader>
      <CardContent>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Select value={timeRange} onValueChange={onTimeRangeChange}>
          ...
          <SelectContent>
            <SelectItem value="1h">{t('timeRanges.1h')}</SelectItem>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {/* Response Time Chart */}
        <MetricChart
          title={t('metrics.averageResponseTime')}
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-4">
            <div className="text-center">
              ...
              <div className="text-sm text-muted-foreground">{t('sla.target')}</div>
            </div>
            <div className="text-center">
              ...
              <div className="text-sm text-muted-foreground">{t('sla.current')}</div>
            </div>
            <div className="text-center">
              <div className={`text-2xl font-bold ${
              ...
              <div className="text-sm text-muted-foreground">{t('sla.monthToDate')}</div>
            </div>
            <div className="text-center">
              <div className={`text-2xl font-bold ${
                (overview?.sla?.breach ?? false) ? 'text-red-600' : 'text-green-600'
              }`}>
                {(overview?.sla?.breach ?? false) ? t('sla.breach') : t('sla.good')}
              </div>
              <div className="text-sm text-muted-foreground">{t('sla.status')}</div>
            </div>
          </div>
        </CardContent>
      </Card>
